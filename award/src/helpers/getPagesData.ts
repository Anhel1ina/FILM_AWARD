...
export const url: string = 'https://api.kinopoisk.dev/v1.4/movie?limit=16'
export const options = {
    method: 'GET',
    headers: { accept: 'application/json', 'X-API-KEY': 'QHX1ENK-J284RF8-H7FB80W-VDQE06A' }
}
...
export const getPages = (
    total: number,
    filmsForFirstPage: number,
    filmsPerPage: number,
    currentPage: number
    ): string[] => {

        const pageCount = total > 11 ? Math.ceil((total - filmsForFirstPage) / filmsPerPage) + 1 : 1
        const pages = []
